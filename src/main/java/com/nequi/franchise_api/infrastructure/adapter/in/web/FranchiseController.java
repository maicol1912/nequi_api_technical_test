...
public class FranchiseController {
    ...
    public Mono<ApiResponse<FranchiseDetailResponse>> getCompleteFranchise(
            @Parameter(description = "Franchise ID", required = true)
            @ValidUuid @PathVariable String franchiseId) {

        log.info("Getting complete franchise: {}", franchiseId);

        return franchiseApplicationService
                .getFranchiseComplete(franchiseId)
                .map(franchiseMapper::toDetailResponse)
                .map(ApiResponse::success)
                .doOnSuccess(result -> log.info("Complete franchise found: {}", result.data().id()));
    }
